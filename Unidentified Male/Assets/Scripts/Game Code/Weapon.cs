...
public abstract class Weapon : MonoBehaviour
{
    ...
    // Method to reduce the durability of the weapon each time it is used in combat, which leads to the weapon breaking when durability reaches zero
    ...
    private void BreakWeapon()
    {
        ...
        // Looks for PlayerCombat class
        // Since the weapon is a child of the Player, we look in the parent
        PlayerCombat combat = GetComponentInParent<PlayerCombat>();

        if (combat != null)
        {
            // Tell PlayerCombat to refresh its list and visuals
            combat.UpdatePlayerAppearance(); 
        }

        // Handles the physical destruction
        Destroy(gameObject);
    }
